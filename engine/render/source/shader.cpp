...
namespace gfx
{
	...
	void Shader::PopulateUniforms ()
	{
		...
		for ( int i = 0; i < UniformsCount; i ++ )
		{
			...
			size_t ArrayBracketPosition = UniformName.find ( '[' );
			if ( ArrayBracketPosition != std::string::npos )
			{
				UniformName.erase ( UniformName.begin() + ArrayBracketPosition,UniformName.end() );
				for ( int j = 0; j <= 256; j ++ )
				{
					std::ostringstream StringBuilder;
					StringBuilder << UniformName << '[' << j << ']';
					UniformLocation = glGetUniformLocation ( m_Handle, StringBuilder.str().c_str() );
					if ( UniformLocation < 0 )
						break;
					m_Uniforms . insert ( { StringBuilder.str(), UniformLocation } );
				}
			}
